...
function FailureBubble({
  ...
  //
  // Partial by design: the backend taxonomy is larger than the set worth
  // writing distinct chat copy for. `resolveFailureReasonKey` degrades a
  // refined reason to its `agent_error` family, so an entry only earns its
  // place here when it can say something the family line can't — usually a
  // different next step (re-auth, top up, check the network). Only a reason
  // with no family copy at all reaches the generic fallback.
