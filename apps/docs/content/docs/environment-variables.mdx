...
| `MULTICA_CODEX_FIRST_TURN_TIMEOUT` | `0` | Explicit override for the Codex first-turn no-progress ceiling; `0` keeps the default (which `MULTICA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` can only shrink, never raise) |
