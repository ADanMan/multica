...
| `MULTICA_DAEMON_HEALTH_PORT` | `19514` (named profiles derive an offset) | Local health/control port the daemon binds; set a distinct value per OS user so two daemons on one host do not collide. Equivalent to `--health-port`; the env var is preferred for persistent daemons because `daemon status`/`stop`/`restart` read it too. Not to be confused with `MULTICA_DAEMON_PORT`, the agent→daemon port injected into a run's environment |
