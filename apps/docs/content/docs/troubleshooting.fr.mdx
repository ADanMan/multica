...
## Les exécutions se bloquent sur macOS après une mise à jour de la CLI

Après la mise à jour de la CLI `multica` installée avec Homebrew, par `brew upgrade` ou par la mise à jour automatique du démon, une exécution qui lit `~/Desktop`, `~/Documents`, `~/Downloads`, iCloud Drive ou un autre dossier protégé peut cesser de progresser : le processus de l'agent n'utilise plus de CPU, et l'exécution ne se termine ni n'échoue.

Les autorisations de confidentialité de macOS (TCC) sont attribuées au démon, et chaque outil lancé par un agent utilise les autorisations du démon. Le binaire de la CLI Homebrew n'est pas signé avec une identité stable : macOS lie donc ces autorisations à ce build précis et à son chemin résolu, par exemple `/opt/homebrew/Cellar/multica/<version>/bin/multica`. Après une mise à jour, macOS considère le nouveau binaire comme une autre application et redemande l'autorisation. L'accès au fichier reste bloqué jusqu'à ce que quelqu'un réponde, et le démon ne peut pas savoir qu'une demande est en attente.

Pour rétablir l'accès :

1. Sur ce Mac, répondez **Autoriser** à la boîte de dialogue « souhaite accéder » en attente. L'exécution bloquée reprend.
2. Pour un Mac sans surveillance, accordez l'accès complet au disque au nouveau binaire. Affichez son chemin avec `realpath "$(command -v multica)"`, puis dans **Réglages Système → Confidentialité et sécurité → Accès complet au disque**, supprimez l'ancienne entrée `multica` et ajoutez ce chemin (appuyez sur ⌘⇧G dans le sélecteur de fichiers pour le coller).
3. Redémarrez le démon avec `multica daemon restart`.

Répétez ces étapes après chaque mise à jour de la CLI, ou démarrez le démon avec `--no-auto-update` pour que les mises à jour n'aient lieu que lorsque vous les lancez. La CLI intégrée à [Multica Desktop](/desktop-app) est signée avec un Developer ID, ses autorisations survivent donc aux mises à jour.
