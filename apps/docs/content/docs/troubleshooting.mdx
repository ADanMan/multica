...
## Runs hang on macOS after a CLI upgrade

After the Homebrew `multica` CLI is upgraded, by `brew upgrade` or by the daemon's auto-update, a run that reads `~/Desktop`, `~/Documents`, `~/Downloads`, iCloud Drive, or another protected folder can stop making progress: the agent's process uses no CPU, and the run neither finishes nor fails.

macOS privacy permissions (TCC) are attributed to the daemon, so every tool an agent starts uses the daemon's grants. The Homebrew CLI binary is not signed with a stable identity, so macOS ties those grants to that exact build at its resolved path, such as `/opt/homebrew/Cellar/multica/<version>/bin/multica`. After an upgrade, macOS treats the new binary as a different app and asks again. The file access blocks until someone answers the prompt, and the daemon cannot tell that a prompt is pending.

To restore access:

1. On that Mac, answer the pending "would like to access" dialog with **Allow**. The stuck run continues.
2. For a Mac that runs unattended, grant Full Disk Access to the new binary. Print its path with `realpath "$(command -v multica)"`, then under **System Settings → Privacy & Security → Full Disk Access** remove the stale `multica` entry and add that path (press ⌘⇧G in the file picker to paste it).
3. Restart the daemon with `multica daemon restart`.

Repeat this after every CLI upgrade, or start the daemon with `--no-auto-update` so upgrades happen only when you run them. The CLI bundled with [Multica Desktop](/desktop-app) is signed with a Developer ID, so its grants survive upgrades.
